...
class MyPVT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.local_conv2 = nn.Conv2d(in_channels=embed_dims[0]+128, out_channels=embed_dims[0], kernel_size=1)
    ...
    def forward_features(self, x):
        ...
        x = self.local_conv2(torch.cat([x, local], dim=1))
...
@register_model
def mypvt20_3_small(pretrained=False, **kwargs):
    model = MyPVT(
...
class MyPVTa(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.local_conv1 = LocalNet(layers=1)
        self.local_conv2 = nn.Conv2d(in_channels=embed_dims[0]+128, out_channels=embed_dims[0], kernel_size=1)
...
@register_model
def mypvt20_3a_small(pretrained=False, **kwargs):
    model = MyPVTa(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
if __name__ == '__main__':
    ...
    model = mypvt20_3_small(drop_path_rate=0.1).to(device)
