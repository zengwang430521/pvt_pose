...
class MyPVTd2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.local2_conv1 = LocalNet()
        # self.local2_fc = nn.Linear(in_features=embed_dims[1]+128, out_features=embed_dims[1])
        self.local2_fc = Mlp_old(in_features=embed_dims[1]+128, hidden_features=embed_dims[1], out_features=embed_dims[1])
...
@register_model
def mypvt20_4d2_small(pretrained=False, **kwargs):
    model = MyPVTd2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
class MyPVTd3(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.local2_conv1 = LocalNet(layers=5)
        self.local2_fc = nn.Linear(in_features=embed_dims[1]+128, out_features=embed_dims[1])
        # self.local2_fc = Mlp_old(in_features=embed_dims[1]+128, hidden_features=embed_dims[1], out_features=embed_dims[1])
...
@register_model
def mypvt20_4d3_small(pretrained=False, **kwargs):
    model = MyPVTd3(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
class MyPVTe(nn.Module):
    ...
    def forward_features(self, x):
        ...
        local = self.local3_conv1(img)
        local = map2token(local, loc)
        x = self.local3_fc(torch.cat([x, local], dim=-1)) + x
...
@register_model
def mypvt20_4e_small(pretrained=False, **kwargs):
    model = MyPVTe(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
class MyPVTf(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.local_conv1 = DynamicConv(3 * local_kernel[1] * local_kernel[1], 128, embed_dims[1], True, 1)
        self.local_act1 = nn.Sequential(
            norm_layer(128),
            act_layer()
        )
        # groups = 128 // 16
        # self.local_conv2 = DynamicConv(128, 128, embed_dims[1], True, groups)
    ...
    def forward_features(self, x):
        ...
        # x = self.local2_fc(torch.cat([x, local], dim=-1)) + x
        B, N, _ = x.shape
        local = extract_local_feature(img, loc, self.local_kernel)
        local = local.flatten(1)
        local = local.reshape(B, N, -1)
        local = self.local_conv1(local, x)
...
@register_model
def mypvt20_4f_small(pretrained=False, **kwargs):
    model = MyPVTf(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
if __name__ == '__main__':
    ...
    model = mypvt20_4f_small(drop_path_rate=0.1).to(device)
