...
def token2critcal(x, loc, k, return_mask=False):
    B, N, C = x.shape
    idx_cri = get_critical_idx(x, k)[:, :, None]
    loc_critical = torch.gather(loc, 1, idx_cri.expand([B, k, 2]))
...
class MyAttention(nn.Module):
    ...
    def forward(self, x, x_source, loc_source, H, W, conf_source=None):
        ...
        if conf_source is not None:
            ...
            x_source = token2critcal(x_source, loc_source, h * w)
...
@register_model
def mypvt20_7a_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt20_7a_small(drop_path_rate=0.1).to(device)
