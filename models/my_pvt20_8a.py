...
'''use critical points replace 7*7 grid in the attention
predicting offset from global feature
'''
...
class MyAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.delta_fc = nn.Linear(dim, 2*7*7)
    ...
    def forward(self, x, x_source, loc_source, H, W, conf_source=None):
        ...
        k = h * w
        delta = self.delta_fc(x_source.mean(dim=1)).reshape(B, k, 2)
        loc_critical = loc_source[:, :k, :] + delta
        loc_critical = loc_critical.clamp(min=-1, max=1)
        if conf_source is not None:
            tmp = token2critcal(torch.cat([x_source, conf_source], dim=-1),
                                loc_source, loc_critical)
            x_source = tmp[:, :, :-1]
            conf_source = tmp[:, :, -1:]
        else:
            x_source = token2critcal(x_source, loc_source, loc_critical)

        x_source = x_source.unsqueeze(2).permute(0, 3, 1, 2)
        x_source = self.sr(x_source)
        x_source = x_source.permute(0, 2, 3, 1).squeeze(2)
        x_source = self.norm(x_source)
        x_source = self.act(x_source)
...
@register_model
def mypvt20_8a_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt20_8a_small(drop_path_rate=0.1).to(device)
