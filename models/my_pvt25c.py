    ...
    def forward(self, x, loc, src, H, W, N_grid):
        ...
        if vis:
            ...
            for i in range(1):
                ...
                ax.clear()
                # conf_map = token2map(conf, loc, [H, W], self.inter_kernel, self.inter_sigma)
                conf_map = token2map(conf, loc, [7, 7], self.inter_kernel, self.inter_sigma)
...
@register_model
def mypvt25c_small(pretrained=False, **kwargs):
    model = MyPVT25c(
...
@register_model
def mypvt2520_small(pretrained=False, **kwargs):
    model = MyPVT2520(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def mypvt2520_2_small(pretrained=False, **kwargs):
    model = MyPVT2520_2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
if __name__ == '__main__':
    ...
    model = mypvt2520_small(drop_path_rate=0.1).to(device)
