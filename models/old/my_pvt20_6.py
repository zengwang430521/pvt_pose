...
'''3x3 conv based on inter points'''
...
class MyDWConv(nn.Module):
    def __init__(self, dim=768):
        super().__init__()
        self.dwconv = TokenInterConv3x3(dim, dim, groups=dim)

    def forward(self, x, loc, H, W, kernel_size, sigma):
        x = self.dwconv(x, loc, H, W)
        return x


class TokenInterConv3x3(nn.Conv2d):
    def __init__(self, dim, dim_out, groups=1, bias=True):
        super().__init__(in_channels=dim, out_channels=dim_out,
                         kernel_size=3, stride=3, bias=bias,
        ...
        loc = torch.stack((loc[1], loc[0]), dim=-1)
        self.loc_delta = loc[None, None]

    def forward(self, x, loc, H, W):
        B, N, C = x.shape
        spatio_step = 2 / H
        self.loc_delta = self.loc_delta.to(x.device)
        loc_9 = loc[:, :, None, None, :] + self.loc_delta * spatio_step     # B, N, 3, 3, 2
        x_9 = inter_points(x, loc, loc_9.reshape(B, -1, 2)).reshape(B, N, 3, 3, C)
        x_9[:, :, 1, 1, :] = x
        x_9 = x_9.permute(0, 1, 4, 2, 3)   # B, N, C, 3, 3
        y = super().forward(x_9.reshape(B*N, C, 3, 3))    # BN, C, 1, 1
        y = y.reshape(B, N, -1)
        return y
...
class DownLayer(nn.Module):
    """ Down sample
    """
    def __init__(self, sample_ratio, embed_dim, dim_out, drop_rate, down_block, use_inter=True):
        super().__init__()
        ...
        if use_inter:
            self.conv = TokenInterConv3x3(embed_dim, dim_out)
        else:
...
@register_model
def mypvt20_6_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt20_6_small(drop_path_rate=0.1).to(device)
