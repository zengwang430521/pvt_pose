...
class MTA_norm(BaseModule):
    def __init__(self,
        ...
        for i in range(self.start_level, self.end_level + 1):
            l_norm = nn.LayerNorm(in_channels[i])
            self.pre_norms.append(l_norm)
    ...
    def forward(self, inputs):
        ...
        for i, lateral_conv in enumerate(self.lateral_convs):
            tmp = copy.copy(inputs[i + self.start_level])
            tmp['x'] = lateral_conv(self.pre_norms[i](tmp['x']).unsqueeze(2).permute(0, 3, 1, 2)).permute(0, 2, 3, 1).squeeze(2)
            input_dicts.append(tmp)
        ...
        for i in range(len(input_dicts) - 2, -1, -1):
            input_dicts[i]['x'] = input_dicts[i]['x'] + token_downup(input_dicts[i], input_dicts[i+1])
            input_dicts[i]['x'] = self.merge_blocks[i](input_dicts[i], gather_dict)
